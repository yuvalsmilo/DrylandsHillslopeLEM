...
from landlab.components.overland_flow import _links as links
    ...
    **Additional References**
    ...
    _info = {
        ...
        "water__specific_discharge": {
            ...
            "units": "m^2 / s",
            "mapping": "link",
            "doc": "The water specific discharge at link",
        },
    ...
    def __init__(
        ...
        try:
            self._h_links = grid.add_zeros(
                "surface_water__depth_at_link",
                at="link",
                units=self._info["surface_water__depth_at_link"]["units"],
            )
        except FieldError:
            self._h_links = grid.at_link["surface_water__depth_at_link"]
    ...
    def calc_time_step(self):
        ...
        al., 2012
        """
        depth_at_link = self.grid.at_link['surface_water__depth_at_link']
        depth_at_node = self._grid.at_node["surface_water__depth"]

        water_depth_of_links_at_node = depth_at_link[self.grid.links_at_node] * self.grid.link_dirs_at_node
        water_depth_of_downwind_link_at_node = np.amax( water_depth_of_links_at_node, axis=1)
        water_depth_of_downwind_link_at_node[water_depth_of_downwind_link_at_node<0]= 0 #negative are in-links
        self._dt = (
            self._alpha
            * self._grid.dx
            / np.sqrt(self._g * np.amax(water_depth_of_downwind_link_at_node ))
        )
    ...
    def overland_flow(self, dt=None):
        ...
        while local_elapsed_time < dt:
            ...
            self._h_links = self._grid["link"]["surface_water__depth_at_link"]
            w  = self._grid["node"]["water_surface__elevation"] # !! Return the water surface elevation at node !!
            ...
            self._grid.at_node["water_surface__elevation"] = self._h + self._z
            ...
            # those errors.
            #print(self._dt)
            #self.grid.st.run_one_step(dt=self._dt)
            if dt is np.inf:
                break
